...
            while (string.IsNullOrWhiteSpace(username))
        ...
        static void SearchAndBookTaxi(Customer c)
        {
            ...
            // show taxies list 
            var availableTaxis = taxis.Where(t => t.IsAvailable).ToList();
            ...
            for (int i = 0; i < availableTaxis.Count; i++)
            {
                var t = availableTaxis[i];
            ...
            if (taxiChoice < 0 || taxiChoice >= availableTaxis.Count)
            {
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("Invalid choice!");
                Console.ResetColor();
            ...
            Taxi selectedTaxi = availableTaxis[taxiChoice];
